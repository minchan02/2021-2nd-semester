...
// Servo range
#define _DUTY_MIN 1500 // [2952] 서보의 최소 각도값 왼쪽으로 올라감 
#define _DUTY_NEU 1780 // [2952] 서보의 중간 각도값 (1750- 1820)
#define _DUTY_MAX 2050 // [1691] 서보의 최대 각도

// Servo speed control
#define _SERVO_ANGLE 40  //[2967] 서보 각도 설정
...
// PID parameters
#define _KP 0.7 // [2957] 비례 제어 값
                // [2961] 비례이득
#define _KD 20 // 미분 제어 값
...
float x_out, x_last = 0, v_current = 0, v_last = 0, a_current = 0, a_last = 0, x_stable, v_stable;
...
////////////////////
  ...
  if(event_dist) { 
     event_dist = false;
      ...
      float v_current = (dist_raw - x_last)/_INTERVAL_DIST;
      float a_current = (v_current - v_last)/_INTERVAL_DIST;
    ...
    // inertial interpolation
      if(abs(a_current) > a_mean * NOISE_COEFF){
        ...
        if(is_interpolate == 0) v_stable = (vel_lookback_mean/lookback_count) * _INTERVAL_DIST + acc_lookback_mean/lookback_count * _INTERVAL_DIST;
        else v_stable = v_stable + acc_lookback_mean/lookback_count * _INTERVAL_DIST;
      ...
      if (dist_ema == 0){                  // [2959] 맨 처음
        dist_ema = x_out;               // [2959] 맨 처음 ema값 = 필터링된 측정값
      }                                    // [2963] dist_ema를 dist_raw로 초기화
      else{
        dist_ema = alpha * x_out + (1-alpha) * dist_ema;   // [2959] ema 구현
      }
