...
float dist_min, dist_max, dist_raw, dist_prev, alpha; // unit: mm
...
// get a distance reading from the USS
  ...
  dist_ema = alpha * dist_raw + ((1-alpha) * dist_ema); // dist_ema 사용
...
if(dist_ema <= 180.0) {
     myservo.writeMicroseconds(_DUTY_MIN);  // 550
     analogWrite(PIN_LED, 0);
  }
  else if(dist_ema < 360.0){ // 거리에 비례하여 0도 ~ 180도 사이 변화
     myservo.writeMicroseconds(550+1850*((dist_ema/180) - 1)); // 1475
     analogWrite(PIN_LED, 255);
  }
  else {
    myservo.writeMicroseconds(_DUTY_MAX); // 2400
    analogWrite(PIN_LED, 0);
  }
